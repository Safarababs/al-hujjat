// EventsHero.jsx
import React from "react";
import { Helmet } from "react-helmet";

const EventsHero = () => {
  return (
    <>
      <Helmet>
        <title>Events | AL Hujjat Online Quran Academy</title>
        <meta
          name="description"
          content="Stay updated on upcoming Quranic events, workshops, and spiritual sessions hosted by AL Hujjat Academy."
        />
      </Helmet>

      <header className="container-fluid hero-header">
        <div className="container">
          <div className="row">
            <div className="col-lg-7">
              <div className="hero-header-inner animated zoomIn">
                <h1 className="display-1 text-dark">Events</h1>
                <nav aria-label="breadcrumb">
                  <ol className="breadcrumb mb-0">
                    <li className="breadcrumb-item">
                      <a href="/">Home</a>
                    </li>
                    <li className="breadcrumb-item">
                      <a href="/pages">Pages</a>
                    </li>
                    <li
                      className="breadcrumb-item text-dark active"
                      aria-current="page"
                    >
                      Events
                    </li>
                  </ol>
                </nav>
              </div>
            </div>
          </div>
        </div>
      </header>
    </>
  );
};

export default EventsHero;
